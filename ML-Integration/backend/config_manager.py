...
DEFAULT_CONFIG = {
    'data_source': 'noaa',  # Default to NOAA instead of local
...
def load_config() -> Dict[str, Any]:
    ...
    data_source = os.getenv('DATA_SOURCE', 'noaa').lower()  # Default to NOAA instead of local
    ...
    if data_source == 'aws':
        ...
        # NOAA data source configuration
        noaa_config = {
            'temp_dir': os.getenv('NOAA_TEMP_DIR'),  # Optional - uses default if not set
            'cache_days': int(os.getenv('NOAA_CACHE_DAYS', '7'))  # Default 7 days
        }
        
        config['aws'] = None
        config['local'] = None
        config['noaa'] = noaa_config
    else:
