...
# Alias for backward compatibility (used by analysis_engine.py)
VESSEL_TYPE_CATEGORIES = VESSEL_TYPES_BY_CATEGORY
...
def filter_dataframe_by_vessel_type(df, vessel_type_codes):
    ...
    Args:
        ...
        vessel_type_codes: List of vessel type codes to include, or single code, or category name(s)
    ...
    if vessel_type_codes is None:
        return df
    
    # If it's a string, check if it's a category name
    if isinstance(vessel_type_codes, str):
        # Check if it's a category name
        if vessel_type_codes in VESSEL_TYPES_BY_CATEGORY:
            vessel_type_codes = get_vessel_types_by_category(vessel_type_codes)
        else:
            # Single code as string, convert to list
            try:
                vessel_type_codes = [int(vessel_type_codes)]
            except (ValueError, TypeError):
                logger.warning(f"Invalid vessel type code: {vessel_type_codes}")
                return df
    
    # Ensure it's a list
    if not isinstance(vessel_type_codes, (list, tuple, set)):
        logger.warning(f"vessel_type_codes must be a list, got {type(vessel_type_codes)}")
        return df
    
    # Convert to list if it's a set or tuple
    vessel_type_codes = list(vessel_type_codes)
    
    # Filter out any None values
    vessel_type_codes = [code for code in vessel_type_codes if code is not None]
    
    if not vessel_type_codes:
        return df
