...
def detect_available_claude_model(api_key: str) -> str:
    ...
    # Haiku > Sonnet (Opus models excluded for cost efficiency)
    # Note: Claude 4 models may not be available yet, so we prioritize 3.5 models
    model_rankings = {
        "claude-3-5-haiku": 900,       # Claude 3.5 Haiku (preferred for speed and cost)
       # "claude-3-5-sonnet": 850,      # Claude 3.5 Sonnet
        "claude-3-haiku": 700,         # Claude 3 Haiku
  #      "claude-3-sonnet": 600,        # Claude 3 Sonnet
        "claude-haiku-4": 1100,        # Claude 4 Haiku (if available)
        #"claude-sonnet-4": 1000,       # Claude 4 Sonnet (if available)
    ...
    # Updated with current available models as of 2025
    fallback_models = [
        ("claude-3-5-haiku-20241022", "Claude 3.5 Haiku (Oct 2024)"),
        ("claude-3-5-haiku-20240620", "Claude 3.5 Haiku (Jun 2024)"),
        ("claude-3-haiku-20240307", "Claude 3 Haiku"),
  #      ("claude-3-5-sonnet-20241022", "Claude 3.5 Sonnet (Oct 2024)"),
 #       ("claude-3-5-sonnet-20240620", "Claude 3.5 Sonnet (Jun 2024)"),
#        ("claude-3-sonnet-20240229", "Claude 3 Sonnet"),
        # Note: Claude 4 models (claude-haiku-4, claude-sonnet-4) may not be available yet
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.debug(f"API returned models data: {models_data}")
            ...
            if "data" in models_data:
                ...
                logger.warning(f"API response missing 'data' field: {models_data}")
        else:
            logger.warning(f"API returned status {response.status_code}: {response.text[:200]}")
        
    except ImportError as e:
        logger.warning(f"requests library not available, using fallback model list: {e}")
        ...
        if hasattr(e, 'response'):
            ...
            logger.debug(f"API response text: {getattr(e.response, 'text', 'unknown')[:200]}")
    ...
    # Track first authentication error for better error messages
    ...
    for model_id, model_name in models_to_try:
        ...
        try:
            ...
            # Authentication error - this is critical, log it but continue trying other models
            logger.warning(f"⚠ Authentication error for {model_name}: {str(auth_error)}")
            # Store first auth error for better error message
            if first_auth_error is None:
                first_auth_error = str(auth_error)
            continue
        except anthropic.APIError as api_error:
            # API errors - log and continue
            logger.warning(f"⚠ API error for {model_name}: {str(api_error)}")
            continue
        except Exception as e:
            ...
            if "insufficient" in error_str or "quota" in error_str:
                ...
                continue
            elif "authentication" in error_str or "unauthorized" in error_str or ("invalid" in error_str and "key" in error_str):
                logger.warning(f"⚠ Authentication issue for {model_name}: {str(e)[:200]}")
                # Store first auth error for better error message
                if first_auth_error is None:
                    first_auth_error = str(e)[:200]
                continue
            else:
                # Unexpected error - log it but continue trying
                logger.warning(f"⚠ Error testing {model_name}: {str(e)[:200]}")
                continue
    
    # No models available - provide detailed error message
    error_msg = (
    ...
    if first_auth_error:
        error_msg += f"\n\nFirst error encountered: {first_auth_error}"
    
    logger.error(error_msg)
    logger.error(f"Tested {len(models_to_try)} models, all failed")
    
    # Log the models that were tested for debugging
    logger.error(f"Models tested: {[m[0] for m in models_to_try]}")
    
    # Provide troubleshooting guidance
    troubleshooting = "\n\nTroubleshooting steps:\n"
    troubleshooting += "1. Verify your API key at https://console.anthropic.com/\n"
    troubleshooting += "2. Check that your API key starts with 'sk-ant-'\n"
    troubleshooting += "3. Ensure your account has access to Claude models\n"
    troubleshooting += "4. Check your account status and billing\n"
    troubleshooting += "5. Try testing the API key directly with curl:\n"
    troubleshooting += "   curl https://api.anthropic.com/v1/messages \\\n"
    troubleshooting += "     -H 'x-api-key: YOUR_KEY' \\\n"
    troubleshooting += "     -H 'anthropic-version: 2023-06-01' \\\n"
    troubleshooting += "     -d '{\"model\": \"claude-3-5-haiku-20241022\", \"max_tokens\": 10, \"messages\": [{\"role\": \"user\", \"content\": \"test\"}]}'"
...
class AISFraudDetectionAgent:
    ...
    def __init__(self, api_key: str, auto_detect_model: bool = True):
        # Initialize client with extended timeout for long-running analyses
        # Default timeout is 60s, we increase to 600s (10 minutes) to handle
        # long data processing operations without connection timeouts
        self.client = anthropic.Anthropic(
            api_key=api_key,
            timeout=600.0  # 10 minutes to handle long analyses
        )
...
- Available data range: January 1, 2021 onwards
...
- You can perform analysis for any date from January 1, 2021 onwards
- If a user asks about dates before January 1, 2021, politely inform them that data is only available from that date forward
- Always suggest dates within the valid range when helping users

⚠️ HANDLING MISSING DATA - CRITICAL
- **IMPORTANT:** Data files may not exist for all dates, even within the valid range
- When analysis tools return `success: false` with an `error` field mentioning missing dates:
  - **DO NOT** panic or apologize excessively - this is normal and expected
  - **DO** clearly inform the user which specific dates are missing data
  - **DO** suggest alternative date ranges that might have data
  - **DO** check the `missing_dates` field in the error response to see exactly which dates failed
  - **DO** continue the conversation naturally - ask if they'd like to try a different date range
- Example response: "I couldn't find data files for [specific dates]. This is normal - data may not be available for every day. Would you like me to try a different date range, or check what dates are available?"
- **NEVER** let missing data errors disrupt or end the conversation - always offer alternatives
...
- Query historical vessel movements and behavior (from January 1, 2021 onwards)
...
- **File Naming Convention:** Files are named by date in format `ais-YYYY-MM-DD.parquet` or `ais-YYYY-MM-DD.csv`
  - Example: `ais-2021-01-15.parquet` contains data for January 15, 2021
...
- **IMPORTANT:** When a user requests analysis for a date (e.g., "January 15, 2021"), the system automatically searches for and loads the file `ais-2021-01-15.parquet` (or `.csv`)
...
**CRITICAL: IMMEDIATE WORKFLOW TRIGGER**
- When a user uses ANY of these keywords/phrases, IMMEDIATELY start the analysis workflow (do NOT ask for clarification first):
  - "analysis", "analyze", "analyzing", "run analysis", "start analysis", "new analysis"
  - "look at", "examine", "check", "investigate", "review"
  - "find anomalies", "detect anomalies", "search for", "scan"
  - Any request that implies data analysis or anomaly detection
- **DO NOT** respond with text first - IMMEDIATELY call `request_date_selection` tool to begin the workflow
- **DO NOT** ask "Would you like to run an analysis?" - if they said "analyze" or "look at", they already want to!
...
1. Use `request_date_selection` tool to show a date picker popup
   - Default: Start 2021-01-01, End (today or recent date)
...
1. Use `request_output_selection` tool to show output format checkboxes
   - Default: ["consolidated_events_csv", "event_map"]
   - Note: "event_map" includes both markers and heatmap overlay (heatmap is no longer a separate option)
...
- Be transparent about limitations and uncertainties"""
        ...
        return [
            {
                ...
                "input_schema": {
                    ...
                    "properties": {
                        "start_date": {
                            "type": "string", 
                            "description": "Start date in YYYY-MM-DD format (must be January 1, 2021 or later)"
                        },
                        "end_date": {
                            "type": "string", 
                            "description": "End date in YYYY-MM-DD format (must be January 1, 2021 or later)"
                        },
                        "geographic_zone": {
                            "type": "object",
                        ...
                        "vessel_types": {
                            "type": "array",
                            "items": {"type": "string"},
                            "description": "Vessel type categories to filter (e.g., ['Cargo', 'Tanker', 'Fishing']). Valid categories: 'WIG', 'Special', 'HSC', 'Special Purpose', 'Passenger', 'Cargo', 'Tanker', 'Other'. If omitted, analyzes all vessel types."
                        }
                    },
            ...
            },
            {
                ...
                "input_schema": {
                    ...
                    "properties": {
                        ...
                        "default_start": {
                            "type": "string",
                            "description": "Default start date (YYYY-MM-DD, default: 2021-01-01)"
                        },
                        "default_end": {
                            "type": "string",
                            "description": "Default end date (YYYY-MM-DD, default: today's date)"
                        },
                        "min_date": {
                            "type": "string",
                            "description": "Minimum selectable date (YYYY-MM-DD, default: 2021-01-01)"
                        },
                        "max_date": {
                            "type": "string",
                            "description": "Maximum selectable date (YYYY-MM-DD, default: today's date)"
                        }
                    }
                }
            },
            {
                ...
                "description": "Trigger a vessel type selection popup showing ALL 58 AIS vessel types grouped by 8 categories (WIG 20-24, Special 30-39, HSC 40-49, Special Purpose 50-59, Passenger 60-69, Cargo 70-79, Tanker 80-89, Other 90-99). Returns comprehensive list with each vessel type code, name, and description. User can select one or multiple types. Use this when user asks to choose/select vessel types.",
                "input_schema": {
                    "type": "object",
                    "properties": {
                        "message": {
            ...
            {
                "name": "check_analysis_memory",
                "description": "Check if system has sufficient memory to process analysis. Assumes 2GB per day. CALL THIS BEFORE starting analysis if date range > 3 days or after user confirms large dataset.",
                "input_schema": {
                    "type": "object",
                    "properties": {
                        "start_date": {
    ...
    async def chat(self, user_message: str, session_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # FIX: Enable prompt caching to reduce costs by 90% and improve response time
    ...
    async def process_tool_results(self, tool_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        """
        # NOTE: Do NOT validate history here! The tool_use blocks we're responding to
        # would be incorrectly identified as "orphaned" since we haven't added the
        # tool results yet. Validation happens in chat() after conversation turns complete.
        ...
        # Prepend progress summary as text content block if available
        if progress_summary:
        ...
        # Add tool results as tool_result content blocks
        import json
        MAX_CONTENT_SIZE = 50000  # 50KB limit per tool result to prevent 413 errors
        
        for result in filtered_tool_results:
            # Convert result to JSON-serializable format
            result_data = result["result"]
            
            # Serialize result - handle nested objects properly
            try:
                # Try to serialize as JSON string first
                if isinstance(result_data, str):
                    serialized_content = result_data
                else:
                    # Convert to JSON string, handling date objects and other non-serializable types
                    serialized_content = json.dumps(result_data, default=str)
                
                # Check size and truncate if too large
                if len(serialized_content) > MAX_CONTENT_SIZE:
                    logger.warning(f"Tool result content too large ({len(serialized_content)} bytes), truncating to {MAX_CONTENT_SIZE} bytes")
                    # Try to create a summary instead of truncating JSON string
                    if isinstance(result_data, dict):
                        # Create a minimal summary
                        summary = {
                            "success": result_data.get("success", False),
                            "message": result_data.get("message", "Tool execution completed"),
                            "note": "Full result too large to include. Check session storage for complete details."
                        }
                        # Add key fields if they exist
                        for key in ['analysis_id', 'anomaly_count', 'total_anomalies', 'error']:
                            if key in result_data:
                                summary[key] = result_data[key]
                        serialized_content = json.dumps(summary, default=str)
                    else:
                        # Fallback: truncate string
                        serialized_content = serialized_content[:MAX_CONTENT_SIZE] + "... [truncated]"
                        
            except (TypeError, ValueError) as e:
                logger.warning(f"Failed to serialize tool result: {e}, using string representation")
                serialized_content = str(result_data)
                # Still check size
                if len(serialized_content) > MAX_CONTENT_SIZE:
                    serialized_content = serialized_content[:MAX_CONTENT_SIZE] + "... [truncated]"
