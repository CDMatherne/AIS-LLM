...
class OptimizedAISFraudDetectionAgent(AISFraudDetectionAgent):
    ...
    def _find_best_model_variant(self, model_type: str) -> str:
        ...
        # Updated: November 17, 2025 - Removed deprecated models
        if model_type.lower() == "haiku":
            variants = [
                "claude-3-5-haiku-20241022",  # Current Haiku (Oct 2024)
                "claude-3-haiku-20240307"     # Fallback Haiku (Mar 2024)
            ]
        elif model_type.lower() == "sonnet":
            variants = [
                "claude-3-5-sonnet-20240620"   # Sonnet (Jun 2024)
                # Note: claude-3-5-sonnet-20241022 returns 404 errors
                # Removed: claude-3-sonnet-20240229 (deprecated/not found)
            ]
        else:
