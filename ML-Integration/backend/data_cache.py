...
class AISDataCache:
    ...
    async def sync_with_source(self, force_full_reload: bool = False) -> Dict[str, Any]:
        ...
        for date_str in sorted(missing_dates):
            try:
                ...
                load_result = self.data_connector.load_date_range(date_str, date_str)
    ...
    def _load_cache_data(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if PYARROW_AVAILABLE:
                # OPTIMIZED: Use pyarrow with optimized read settings for faster loading
                table = pq.read_table(
                    self.data_cache_file,
                    use_pandas_metadata=True,  # Preserve pandas metadata
                    use_threads=True  # Parallel reading
                )
                df = table.to_pandas()
            else:
                df = pd.read_parquet(self.data_cache_file, engine='pyarrow' if PYARROW_AVAILABLE else 'fastparquet')
    ...
    def _save_cache_data(self, df: pd.DataFrame):
        ...
        try:
            ...
            # OPTIMIZED: Pre-sort by BaseDateTime and MMSI for faster queries later
            if 'BaseDateTime' in df_optimized.columns:
                df_optimized = df_optimized.sort_values(['BaseDateTime', 'MMSI']).reset_index(drop=True)
                logger.debug("Pre-sorted cache data by BaseDateTime and MMSI for faster queries")
            ...
            if PYARROW_AVAILABLE:
                ...
                pq.write_table(
                    ...
                    row_group_size=100000  # Optimize row group size for faster reads
                )
            else:
