...
from analysis_engine import AISAnalysisEngine
from geographic_tools import GeographicZoneManager
from data_connector import AISDataConnector
from data_cache import AISDataCache
from session_manager import SessionManager
from gpu_support import get_gpu_info, get_installation_instructions
from export_utils import AISExporter
from visualization_engine import viz_engine
from map_creator import map_creator
from chart_creator import chart_creator
from temp_file_manager import temp_manager, cleanup_session
...
# Import all handler modules to register their tools
import tool_handlers.analysis_handlers
import tool_handlers.geographic_handlers
import tool_handlers.chart_handlers
import tool_handlers.map_handlers
import tool_handlers.export_handlers
import tool_handlers.visualization_handlers
import tool_handlers.ui_handlers
...
def load_config_from_env() -> Optional[Dict[str, Any]]:
    ...
    data_source = os.getenv('DATA_SOURCE', 'aws').lower()
    ...
    if data_source == 'aws':
        aws_config = {
            ...
            'prefix': os.getenv('AWS_PREFIX', '').rstrip('/')
        }
        ...
        local_config = {
            ...
            'file_format': os.getenv('LOCAL_FILE_FORMAT', 'auto')
        }
...
def _get_gpu_recommendations(gpu_info):
    """Provide recommendations based on GPU status"""
    if not gpu_info['available']:
        return {
            'status': 'no_gpu',
            'message': 'No GPU detected. System will use CPU processing.',
            'suggestions': [
                'For faster processing, consider using a system with NVIDIA or AMD GPU',
                'NVIDIA GPUs with CUDA provide best performance',
                'AMD GPUs with ROCm support also supported (primarily Linux)'
            ]
        }
        ...
        return {
            ...
            'suggestions': [
                f"Using {gpu_info['device_count']} NVIDIA GPU(s)",
                'RAPIDS libraries provide maximum acceleration',
                'Large datasets will process significantly faster'
            ]
        }
        ...
        return {
            ...
            'suggestions': [
                f"Using {gpu_info['device_count']} AMD GPU(s)",
                'ROCm/HIP support enabled',
                'Performance good, though NVIDIA CUDA typically faster' if gpu_info['platform'] == 'Linux' 
                else 'Windows support is experimental'
            ]
        }
...
async def setup_from_env():
    ...
    try:
        ...
        # Create Claude agent
        agent = AISFraudDetectionAgent(config['claude_api_key'])
...
async def get_cache_status():
    """Get data cache status and statistics"""
    global data_cache
    
    if not data_cache:
        return {
            "enabled": False,
            "message": "Data cache not initialized (local data source or no config)"
        }
    ...
    return {
        "enabled": True,
        "stats": stats,
        "cached_dates": sorted(data_cache.get_cached_dates())
    }


@app.post("/api/cache/sync")
async def sync_cache(force: bool = False):
    """Manually trigger cache sync with data source"""
    global data_cache
    
    if not data_cache:
        return {
            "success": False,
            "error": "Data cache not initialized"
        }
    
    try:
        result = await data_cache.sync_with_source(force_full_reload=force)
        return result
    except Exception as e:
